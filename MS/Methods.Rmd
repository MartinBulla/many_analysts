...
Please see [here](https://raw.githack.com/MartinBulla/many_analysts/data_exploration/R/data_exploration_and_analytical_decisions.html) for our entire data exploration workflow.

For the paired analyses we have thus used only the pairs that differ in the original brood size by no more than one chick (75% of nest pairs, N = 303).
...
We specified the change in chick number in the rearing nests (`change_chick_n`) as a difference between the number of chicks at the start of the experiment (`rear_Cs_at_start_of_rearing`) and number of chicks on day 14 of the experiment (`d14_rear_nest_brood_size`) and fitted this variable as response. The number of chicks to enlarge or reduce the brood (`net_rearing_manipulation`; range -4 to 4, median 0; note that zero indicates the control nests) and brood sex ratio (`brood_sex_ratio`; range 0-1, median 0.5) as explanatory variables. Brood sex ratio was calculated as proportion of males in the nest (1 = all males, 0 = all females), while indicating unknown sex of chicks as 0.5. We specified the observer (`day14_measurer`), the compartment of the study area (`rear_area`), the year (`hatch_year`) and the day on which the rear nest hatched left centered within year, i.e. minimum day within each year subtracted from year hatch day of that given year (`rear_nest_OH_l`) as random intercepts. In total, we had 357 nests forwhich these data were available. 

#### Model 1 - chick mortality
```r
    lmer(change_chick_n ~ 
...
#### Model 3 - tarsus *with* interaction between sex and number of chicks moved
```{r, eval=FALSE, echo=TRUE}
    lmer(day_14_tarsus_length ~ 
         net_rearing_manipulation * chick_sex_molec + 
         brood_sex_ratio +
...
#### Model 3 - weight *with* interaction between sex and number of chicks moved
```{r, eval=FALSE, echo=TRUE}
    lmer(day_14_weight ~ 
         net_rearing_manipulation * chick_sex_molec + 
         brood_sex_ratio +
...
#### Model 3 - body mass index *with* interaction between sex and number of chicks moved
```{r, eval=FALSE, echo=TRUE}
    lmer(body_mass_index 
         net_rearing_manipulation * chick_sex_molec + 
         brood_sex_ratio +
...
Note that the above described models differ from those proposed in our [a priori protocol](https://rawcdn.githack.com/MartinBulla/many_analysts/3601f2f03b574d8ca45a53d60a12f100d38b707b/MS/Protocol.html) because we followed our a priori decisions about analyses. There were not enough data for the number of chicks at the start of the experiment ('rear_Cs_at_start_of_rearing') and hence we have not used it in the models. The correlation between 'net_rearing_manipulation' and 'd14_rear_nest_brood_size' was 0.68, i.e. higher than our predefined 0.6 threshold). Thus, according to the [a priori protocol](https://rawcdn.githack.com/MartinBulla/many_analysts/3601f2f03b574d8ca45a53d60a12f100d38b707b/MS/Protocol.html) we used only one variable. As net change in chick number captures the actual experimental manipulation, we use this variable through out. Note however, that we have also fitted a model with 'd14_rear_nest_brood_size' instead of 'net_rearing_manipulation', which generated same results (see [Table T, Table W and Table B](https://github.com/MartinBulla/many_analysts/tree/master/Output). 
...
To explore whether the results are consistent even if we specify the models regardless of our a priori assumptions, we have fitted the Chick size models adding `d14_rear_nest_brood_size` to the above described model and another set of models specifying interaction between  `net_rearing_manipulation` and `d14_rear_nest_brood_size`. These results (see Table Te, Table We, Table Be) were similar to those from the initial, simple main text models ([Table T, Table W and Table B](https://github.com/MartinBulla/many_analysts/tree/master/Output)). We have also fitted the intended *a priori* defined models that control for experimental pairs (i.e., reduced, enlarged). As not all data in the dataset had pairs assigned and as this method does not work for control nests, we have refitted the Table T, W and B models to the reduced data and included random slope pair_ID. These results are similar to those from the initial models and can be found in Tables with E suffix (TE, WE, BE).
