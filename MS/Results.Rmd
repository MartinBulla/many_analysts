...
```
We found consistent treatment effects across all response variables that were independent of the model specifications (see Tables TE, WE and BE with all specified models and standardized  effect sizes [here](https://www.dropbox.com/sh/fmyp12uu45qruwo/AACkHcZAZvqYL0Z0Z1ZIShF1a?dl=0)). Tn the nests where most chicks were added, which also generally corresponds to the nests with the most chicks at the day 14 (the day of chick measurement), chicks died the most (estimate [95%CI]: 0.236 [0.172 - 0.302] dead chicks per added chick), had smaller tarsus (-0.063 [-0.074 to -0.051] mm per added chick), lower body mass (-0.182 [-0.213 to -0.153] g per added chick), and were the leanest, i.e. had the lowest body mass index (-0.00037 [-0.00045 to -0.00029] per added chick),  (Tables M, Ta, Tb, Wa, Wb, Ba, Wb, and Figures M, Ta, Wa, Ba). Analyses that controlled for on nest pairs generated similar results (see Tables [TEP, WEP, BEP](https://www.dropbox.com/sh/fmyp12uu45qruwo/AACkHcZAZvqYL0Z0Z1ZIShF1a?dl=0), and Figure EP below).

Treatment in interaction with sex did not improve the model fit (see Tables Tc, Wc, Bc below) and the simplest models (model "a" in [Tables T, W, B](https://www.dropbox.com/sh/fmyp12uu45qruwo/AACkHcZAZvqYL0Z0Z1ZIShF1a?dl=0)) performed the best of all (also exploratory) models, i.e. no other model had delta AIC smaller than 5 ( [predefined a priori](https://raw.githack.com/MartinBulla/many_analysts/master/MS/Protocol.html)), and even smaller than 2 (see [Tables TE, WE, BE](https://www.dropbox.com/sh/fmyp12uu45qruwo/AACkHcZAZvqYL0Z0Z1ZIShF1a?dl=0)).

Note that males tend to have larger tarsi and are heavier (regardless of the treatment), but their body mass index is similar to the one of females (Figures T, W, B) - indicating that the body mass of each sex is proportional to their structural size and that the treatment does indeed affect only tarsus size or only body mass.
...
#### Table M: effect sizes for chick mortality (*Model M*)
```{r, echo=FALSE}
o_m %>% 
...
##### Figure Ta: Predicted relationship between day 14 tarsus length and net change in brood size according to chick sex (prediction and its 95% CI)
...
##### Table Ta: effect sizes for chick tarsus model
```{r, echo=FALSE}
...
##### Table Tb: effect sizes for chick tarsus model
```{r, echo=FALSE}
...
##### Table Tc: effect sizes for chick tarsus model
```{r, echo=FALSE}
...
##### Figure W: Predicted relationship between day 14 weight and net change in brood size according to chick sex (prediction and its 95% CI)
```{r, echo=FALSE, cache=TRUE}
...
##### Table Wb: effect sizes for chick weight model
```{r, echo=FALSE}
...
##### Table Wc: effect sizes for chick weight model
```{r, echo=FALSE}
...
##### Figure B: Predicted relationship between day 14 body mass index and net change in brood size according to chick sex (prediction and its 95% CI)
```{r, echo=FALSE, cache=TRUE}
...
##### Table Bb: effect sizes for chick body mass index model
```{r, echo=FALSE}
...
##### Table Bc: effect sizes for chick body mass index model
```{r, echo=FALSE}
...
mortality_plot <-
  ...
  coord_fixed(ratio = 1) +
  xlab(expression(paste("Number is dead chicks in ", italic("REDUCED")," brood (± SE)"))) +
  ylab(expression(paste("Number is dead chicks in ", italic("ENLARGED")," brood (± SE)"))) +
